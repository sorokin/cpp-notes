...
% TODO: убрать все "мы", "нам"

% TODO: убрать слово дотянуться
...
% TODO: что значит интересует?
% TODO: как вариант ля-ля-ля не учитывается
...
\subsection{Перегрузка и шаблонные функции}
	...
	% TODO: можно сказать, что это правило возникло не само на примере этого кода:
	template <typename C>
...
\subsection{Использование SFINAE для ограничения при компиляции}
    ...
    % TODO: Механизм SFINAE можно использоваться, что накладывать ограничения на типы параметров шаблона. Например TODO
...
% TODO: можно это сделать используя дополнительную структуру
template <typename T>
...
% TODO: проговорили как работает

% всё супер но писать класс enable_lalala не прикольно
% говорим, что есть is_signed/is_unsigned
% говорим, что есть enable_if

% TODO: enable_if может быть написан в трех местах: параметры, возвращаемое значение, дефолтные темплейтные аргументы
template <typename T>
...
% сравнить их.
% 1. дефолтные темплейтные аргументы не являются частью сингнатуры -- последствия
% 2. параметры -- сквозь enable_if нельзя дедюсить
template <typename T>
bool is_negative(std::enable_if<is_signed<T>, T>::type val);
% 3. возвращаемое значение -- нельзя написать у конструктора

% Как пример использования destroy_storage.
