...
Его суть заключается в том, что в C++11 ошибки подстановки выражений в сигнатуре функций трактуются как ошибки SFINAE, а не как hard-ошибки.
...
Это эффективно в смысле временных затрат на компиляцию, по сравнению с использованием enable\_if в сигнатуре функции.\\\\
...
Не участвуют в линковке, так как нельзя разделить объявление и определение \textbf{constexpr} переменной. Подставляются во все места использования подобно \textbf{\texttt{inline}}.
...
\end{enumerate}
Возможна рекурсия. \\\\
По стандарту \textbf{constexpr} функции неявно являются \texttt{inline}. Не могут иметь внешнюю линковку. \\\\
Пример:
\begin{minted} [linenos, frame=lines, framesep=2mm, tabsize = 4, breaklines]{c++}
...
Дело в том, что в версии с \textbf{if constexpr} поверка осуществляется на более поздней стадии - на стадии, уже внутри тела функции, когда код уже сгенерирован.
