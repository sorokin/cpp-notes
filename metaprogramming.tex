...
Но у этого метода есть недостаток. Так как значения по умолчанию не входят в сигнатуру функции, невозможно перегружать функции отличающиеся лишь SFINAE-проверками.
...
Переменные помеченные как const не обязательно являются constant expression'ами. Например:
\begin{minted} [linenos, frame=lines, framesep=2mm, tabsize = 4, breaklines]{c++}
int compute_value()
{
    int result;
...
\end{minted}
В данном случае const означает лишь то, что переменную невозможно изменять. Конкретное значение этой переменной известно лишь после запуска программы. const переменная является constant expression только если:
\begin{enumerate}
...
Введенное в C++11 ключевое слово constexpr решает две задачи:
\begin{enumerate}
\item Для переменных --- позволяет указать, что переменная является constant expression.
\item Для функций --- позволяет указать, что результат вызова функции является constant expression.
\end{enumerate}
...
% TODO: написать для С++14. Ограничения C++11 не очень интересны.
Объявление функции как \textbf{constexpr} гарантирует, что результат её вызова является \textit{constant expression}
% TODO: не является, например, если переданные аргументы не constant expression. Уточнить формулировку
и, как следствие, ее значение может быть посчитано на этапе компиляции. \\\\
...
% TODO: почему уомпилятор не может соптимизировать обычный if?
