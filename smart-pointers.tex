...
\end{minted}
% TODO: это не копирование
% TODO: а что делать для нулевых укаателей?
...
\end{minted}
% TODO: стрелочка есть, а звездочка?
...
% TODO: не всегда, а что если используют make_shared
...
% TODO: по каким? "... эффективнее shared_ptr поскольку ..."
эффективнее \mintinline{c++}{shared_ptr}, и если планируется использование разделяемого smart-pointer’а для класса, реализуемого самим пользователем, то лучше юзать \mintinline{c++}{intrusive_ptr}, а в случае если приходится иметь дело с библиотечным классом, то \mintinline{c++}{std::shared_ptr}.
...
\end{minted}
% TODO: зачем нужен такой make_shared? Зачем вообще нужен make_shared?
...
% TODO: "экономят аллокации памяти " -- только для make_shared, а не для любого make_xxx!
