...
Этот код --- некорректный, т.к. оба умных указателя указывают на один и тот же объект p, но при вызове конструктора 

\subsection{Сast\_pointer}
Что выполнять безопасное приведение умных указателей можно искользовать следующий синтаксис:
\begin{minted}[linenos, frame=lines, framesep=2mm, tabsize = 4, breaklines]{c++}
struct A {/*...*/};
struct B: A {/*...*/};
...
Функция intrusive_ptr_add_ref вызывается, когда требуется увеличить счетчик ссылок. Функция intrusive_ptr_release вызывается тогда, когда требуется его уменьшить и если он достигает нуля, то требуется самостоятельно удалить объект.
