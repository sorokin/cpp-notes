...
Члены \mintinline{c++}{prev} и \mintinline{c++}{next} помечены как \mintinline{c++}{mutable}, поскольку даже у константных \mintinline{c++}{linked_ptr}'ов они могут изменяться. Это происходит, например, при появлении и удалении соседей.\\
На рисунке \ref{fig:linked_ptr_chain} приведен фрагмент двусвязного списка, образованного \mintinline{c++}{linked_ptr}'ами \mintinline{c++}{ptr1}, \mintinline{c++}{ptr2}, \mintinline{c++}{ptr3}, указывающими на общий объект \mintinline{c++}{object}.
...
void draw_box(real x0, real x1, int i, real y0 = box_y0, real y1 = box_y1, pen color=royalblue) {
    ...
    fill(ptr, color);
    draw(ptr, black);
    label(scale(0.7) * ("ptr" + string(i) + ""), (x0 + box_label_x_offset, y0 + box_label_y_offset), white);
}
...
fill(obj, deepgreen);
draw(obj, black);
label(scale(0.7) * "object", (obj_label_x, obj_label_y), white);
...
draw_last_arrows();
\end{asy}
\caption{Фрагмент двусвязного списка, образованного \mintinline{c++}{linked_ptr}'ами ссылающимися на общий объект}
\label{fig:linked_ptr_chain}
\end{figure}

Конструктор копирования \mintinline{c++}{linked_ptr}'а вставляет новый \mintinline{c++}{linked_ptr} в тот же список где был исходный. На рисунек \ref{fig:linked_ptr_chain_inserted} показан результат копирования \mintinline{c++}{linked_ptr} \mintinline{c++}{ptr2}.
...
\end{asy}
\caption{Измененный список, полученный выполнением операции копирования \mintinline{c++}{linked_ptr<int> ptr4 = ptr2;} из списке показаном на рисунке \ref{fig:linked_ptr_chain}}
\label{fig:linked_ptr_chain_inserted}
\end{figure}
...
\item \mintinline{c++}{linked_ptr} производит больше присваиваний и сравнений на каждую операцию копирования и присваивания, в то время как \mintinline{c++}{std::shared_ptr} лишь инкрементирует счетчик.
\end{enumerate}
